//首页控制器
app.controller('indexController',function($scope,loginService,userService,addressService){
	//部署时，4处跳转需要修改
...
//		location.href = "http://localhost:9035/pay.html#?orderId="+tbOrderId;
	}
	
	$scope.updatePassword=function(isUpdatePassword){
//	    $scope.user.provinceId =$scope.provice.name
//	    $scope.user.cityId = $scope.city.name
//	    $scope.user.townId = $scope.country.value
//	    console.log("password"+$scope.password)
//	    console.log("confirmPassword"+$scope.confirmPassword)
	    if($scope.password==$scope.confirmPassword){//密码一致
	    	$scope.user.password = $scope.password
	    	console.log($scope.user)
			userService.update($scope.user,isUpdatePassword).success(
					function(response){
//						alert(response.message)
						alert("密码修改成功")
//						location.href = "http://192.168.0.200:8082/cas/logout";
					}
			)
	    }else{//密码不一致
	    	alert("两次密码不一致，请重新输入")
	...
	$scope.updateTbOrderStatus=function(tbOrderId,status){
		userService.updateTbOrderStatus(tbOrderId,status).success(
			function(response){
...
//				console.log(response.message);
    ...
    //home-index.html/home-order-pay.html/home-order-receive.html
	$scope.getIndex=function(){
		//查用户名
		loginService.showName().success(
			function(response){		
				$scope.loginName=response.loginName;
				userId=response.loginName;
...
//							console.log(response)
	...
	//取消订单
	$scope.delOrderByOrderId=function(id){
		userService.delOrderByOrderId(id).success(
				function(response){
					if(response.success){
						$scope.getIndex();
					}else{
	...
	$scope.getInfo=function(){
		...
		loginService.showName().success(
			function(response){		
				...
				//查用户
				userService.findUserByUserId(userId).success(
						function(response){	
							console.log(response)
							$scope.user=response
	...
	$scope.update=function(){
	    ...
	    console.log($scope.user)
		userService.update($scope.user,'0').success(
				function(response){
//					alert(response.message)
	...
	$scope.getAddr=function(){
		...
		loginService.showName().success(
			function(response){		
				...
				//查订单
				userService.findAddrByUserId(userId).success(
						function(response){	
//							console.log(response)
	...
	$scope.saveAddr=function(){
	    ...
	    $scope.address.provinceId =$scope.provice.name
	    $scope.address.cityId = $scope.city.name
	    $scope.address.townId = $scope.country.value
	    $scope.address.userId = $scope.loginName
	    
		if($scope.address.id==null){//增加
//			console.log("增加")
...
//			console.log("修改")
		    addressService.update($scope.address).success(
					function(response){
					}
			)
		}
